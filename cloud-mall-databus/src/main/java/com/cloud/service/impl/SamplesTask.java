...
import com.cloud.config.SamplesCanalConfig;
...
@Component
@Slf4j
public class SamplesTask implements Runnable, ApplicationContextAware {

    private ApplicationContext applicationContext;

    @Autowired
    @Qualifier("samplesConnector")
    CanalConnector connector;

    @Autowired
    SamplesCanalConfig canalConfig;
            ...
            while (true) {
                ...
                if (batchId == -1 || size == 0) {
                    ...
                    for (String row :results) {
                        System.out.println("[SamplesTask推送到Kafka]: " + row);
                        this.kafkaSender.send("samples", row);
                    }
                }
                // 提交确认
